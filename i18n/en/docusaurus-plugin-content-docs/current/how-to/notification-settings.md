...
This document describes how to create a notification channel in Gita that allows the system to send automatic alerts according to the rules defined for the cluster. This ensures that critical events, warnings or important information are delivered to those responsible at the right time. Channel configuration involves selecting events, receiving periods and the means by which the notification will be sent (e.g., Mattermost, Slack, Webhook, etc.).
...
Start creating the notification channel.  
...
- Select the events you want to associate with the channel based on criticality.  
...
### 9. Configure Notification Receiving Period

- Define the days of the week and times when you want to receive notifications.  
...
- Destination: Email, Webhook URL or other means of receipt.  
...
5. Modify the desired parameters, such as:
   ...
   - Rules for notification trigger period.
